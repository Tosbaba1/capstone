class OpenLibraryClient
  ...
  def self.search_books(query)
    response = HTTP.get(BASE_SEARCH_URL, params: { q: query, key: ENV['book_search_key'] })
    JSON.parse(response.to_s)
  end

  def self.fetch_work(work_id)
    response = HTTP.get("#{BASE_WORKS_URL}/#{work_id}.json", params: { key: ENV['works_key'] })
    JSON.parse(response.to_s)
  end

  def self.cover_url(cover_id, size)
    "#{BASE_COVER_URL}/#{cover_id}-#{size}.jpg?key=#{ENV['covers_key']}"
  end
end
